...
void set_up_server_socket(sockaddr_in& dest,int& consocket,int& main_socket,int Portnum)
{
  ...
  fl = setsockopt(main_socket, SOL_SOCKET, SO_RCVBUF, (char*)&winsize, sizeof(int));
  if (fl<0) { error("set_up_socket:setsockopt");  }

  fl = setsockopt(main_socket, SOL_SOCKET, SO_SNDBUF, (char*)&winsize, sizeof(int));
  if (fl<0) { error("set_up_socket:setsockopt");  }
...
void set_up_client_socket(int& mysocket,const char* hostname,int Portnum)
{
  ...
  int winsize = 32 * 1024 * 1024;
  fl = setsockopt(mysocket, SOL_SOCKET, SO_RCVBUF, (char*)&winsize, sizeof(int));
  if (fl<0) { error("set_up_socket:setsockopt");  }

   fl = setsockopt(mysocket, SOL_SOCKET, SO_SNDBUF, (char*)&winsize, sizeof(int));
  if (fl<0) { error("set_up_socket:setsockopt");  }
