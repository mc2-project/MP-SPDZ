...
*   Party 0: ./bench_spdz_square.x -N 2 -l 64 -h x.x.x.x -p 0
*   Party 1: ./bench_spdz_square.x -N 2 -l 64 -h x.x.x.x -p 1
*   (to support floating points with sufficient space, we need to set -l to a higher value)
*
* How to benchmark for three parties?
*   Party 0: ./bench_spdz_square.x -N 3 -l 64 -h x.x.x.x -p 0
*   Party 1: ./bench_spdz_square.x -N 3 -l 64 -h x.x.x.x -p 1
*   Party 2: ./bench_spdz_square.x -N 3 -l 64 -h x.x.x.x -p 2
*
...
void generate_one_batch(PlaintextVector<FD> &a, PlaintextVector<FD> &c, const int batch_size, const FHE_Params & params, const FD &FieldD, const PlainPlayer &P, FHE_PK& pk, FHE_SK *psk, ThreadPool * pool){
	PRNG G;
	G.ReSeed();
	...
	/*
	* Step 1: Generate a_i and f_i randomly
	*/
	printf("sampling randomized a/f.\n");
	start_timer_map["sampling randomized a/f"][0] = std::chrono::system_clock::now();
	{
	...
	end_timer_map["sampling randomized a/f"][0] = std::chrono::system_clock::now();
	printf("sampling randomized a/f done.\n");

	/*
	* Step 2: Encrypt a_i/b_i/f_i and prepare to send it out to the first party
	*/
	printf("encrypting a/f.\n");
	start_timer_map["encrypting a/f"][0] = std::chrono::system_clock::now();
	...
	end_timer_map["encrypting a/f"][0] = std::chrono::system_clock::now();
	printf("encrypting a/f done.\n");
	...
	/*
	* Step 3:
	* For the first party, receiving others' encrypted a_i/f_i, adding them together, multiplying a_i and a_i, adding f_i, and sending the resulting ciphertexts back (separate rounds)
	* For the rest of the parties, sending encrypted a_i/f_i and receiving the masked products back (separate rounds)
	*/
	if(my_num == 0){
		...
		start_timer_map["receiving encrypted a/f"][0] = std::chrono::system_clock::now();
		vector<future<void>> res;
		for(int j = 1; j < num_players; j++){
			int party = j;
			res.push_back(pool->enqueue([party, &P, &Ca_others, &Cf_others, batch_size, CaSize, CfSize]() {
				...
				printf("party0: receiving encrypted a/f from party %d.\n", party);
				auto socket_recv = P.socket(party);
				CaStream_other.Receive(socket_recv);
				CfStream_other.Receive(socket_recv);
				printf("party0: receiving encrypted a/f from party %d done.\n", party);

				for(int i = 0; i < batch_size; i++){
					Ca_others[party][i].unpack(CaStream_other);
				}
				for(int i = 0; i < batch_size; i++){
					Cf_others[party][i].unpack(CfStream_other);
				}
			}));
		}
		joinNclean(res);
		end_timer_map["receiving encrypted a/f"][0] = std::chrono::system_clock::now();

		printf("party0: adding encrypted a/f.\n");
		start_timer_map["adding encrypted a/f"][0] = std::chrono::system_clock::now();
		#pragma omp parallel for
		for(int i = 0; i < batch_size; i++){
			for(int j = 1; j < num_players; j++){
		...
		end_timer_map["adding encrypted a/f"][0] = std::chrono::system_clock::now();
		printf("party0: adding encrypted a/f done.\n");
		...
		printf("sending encrypted a/f to party0.\n");
		start_timer_map["sending encrypted a/f"][0] = std::chrono::system_clock::now();
		auto socket_send = P.socket(0);
		CaStream_other.Send(socket_send);
		CfStream_other.Send(socket_send);
		end_timer_map["sending encrypted a/f"][0] = std::chrono::system_clock::now();
		printf("sending encrypted a/f to party0 done.\n");
...
void check_first_result(PlaintextVector<FD> &a, PlaintextVector<FD> &c, const FHE_Params & params, const FD &FieldD, const PlainPlayer &P, ThreadPool * pool){
	(void)(params);
	...
	if(my_num != 0){
		octetStream acStream;
		acStream.resize_precise(a[0].report_size(USED) + c[0].report_size(USED));
		acStream.reset_write_head();

		a[0].pack(acStream);
		c[0].pack(acStream);

		P.comm_stats["Sending directly"].add(acStream);
		auto socket_send = P.socket(0);
		acStream.Send(socket_send);
		P.sent += acStream.get_length();
	}else{
		...
		int rowsize = a[0].report_size(USED) + c[0].report_size(USED);
		...
		for(int j = 1; j < num_players; j++){
			int party = j;
			res.push_back(pool->enqueue([party, rowsize, &P, &a_other, &c_other](){
				octetStream acStream_other;
				acStream_other.resize_precise(rowsize);
				acStream_other.reset_write_head();

				auto socket_recv = P.socket(party);
				acStream_other.Receive(socket_recv);

				a_other[party].unpack(acStream_other);
				c_other[party].unpack(acStream_other);
			}));
		}
		joinNclean(res);
...
int main(int argc, const char** argv)
{
	...
	*/
	...
	generate_one_batch(a, c, batch_size, params, FieldD, P, *ppk, psk, &pool);
	end_time = std::chrono::system_clock::now();

	check_first_result(a, c, params, FieldD, P, &pool);
