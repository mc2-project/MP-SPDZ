...
*   Party 0: ./bench_lowgear_square.x -N 2 -l 64 -h x.x.x.x -p 0
*   Party 1: ./bench_lowgear_square.x -N 2 -l 64 -h x.x.x.x -p 1
*   (to support floating points with sufficient space, we need to set -l to a higher value)
*
* How to benchmark for three parties?
*   Party 0: ./bench_lowgear_square.x -N 3 -l 64 -h x.x.x.x -p 0
*   Party 1: ./bench_lowgear_square.x -N 3 -l 64 -h x.x.x.x -p 1
*   Party 2: ./bench_lowgear_square.x -N 3 -l 64 -h x.x.x.x -p 2
*
...
void generate_one_batch(PlaintextVector<FD> &a, PlaintextVector<FD> &c, const int batch_size, const FHE_Params & params, const FD &FieldD, const PlainPlayer &P, vector<FHE_PK>& pks, FHE_SK *psk, ThreadPool * pool){
	PRNG G;
	G.ReSeed();
...
int main(int argc, const char** argv)
{
	...
	*/
	...
	generate_one_batch(a, c, batch_size, params, FieldD, P, pks, psk, &pool);
